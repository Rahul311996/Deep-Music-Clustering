...
class autoencoder(torch.nn.Module):
    ...
    def forward(self,x):
        out=F.avg_pool2d(F.relu(self.co1_bn(self.conv1(x))),2)
        out=F.avg_pool2d(F.relu(self.co2_bn(self.conv2(out))),2)
        out=F.relu(self.co3_bn(self.conv3(out)))
        out=F.relu(self.co4_bn(self.conv4(out)))
        out=out.view(out.size(0),-1)
        out1=F.relu(self.dense(out))
        out=out1.view(-1,512,1,1)
        out=F.relu(self.deco1_bn(self.deco1(out)))
        out=F.relu(self.deco2_bn(self.deco2(out)))
        out=F.relu(self.deco3_bn(self.deco3(out)))
        out=F.relu(self.deco4_bn(self.deco4(out)))
        out=F.relu(self.deco5_bn(self.deco5(out)))
        out=F.relu(self.deco6_bn(self.deco6(out)))
        out=self.deco7(out)
        return out

device=torch.device('cuda')
model=autoencoder()
model.load_state_dict(torch.load('model_zero_Grad_512.pth'))
model=model.to(device)

criterion=torch.nn.MSELoss()
optim=torch.optim.Adam(model.parameters(),lr=1e-03)

encodings=np.ones((512,1000),dtype=np.float32)
for i in range(1000):
    epoch_loss=0
...
#np.save('encodings_1024_normalised.npy',encodings)
